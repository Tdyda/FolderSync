...
namespace FolderSync.Core.Tests
{
    public class CopyEngineTests
    {
        private static CopyEngine CreateEngine(IFileSystem fs, IFileCopier copier)
            => new CopyEngine(fs, copier, new NullLogger<CopyEngine>());
        ...
        private static OperationStats Stats() => new OperationStats();
        ...
        public async Task Run_Creates_Missing_Directories()
        {
            ...
            var stats = await engine.Run(src, dst, Array.Empty<string>(), Array.Empty<string>(), toCreate, Stats(), CancellationToken.None);
        ...
        public async Task Run_Copies_Files_Increments_FilesCopied_Sets_Timestamps()
        {
            ...
            copier.Verify(c => c.AtomicCopyAsync(J(fs, srcRoot, "a.txt"), J(fs, dstRoot, "a.txt"), It.IsAny<CancellationToken>()), Times.Once);
            copier.Verify(c => c.AtomicCopyAsync(J(fs, srcRoot, "deep", "b.bin"), J(fs, dstRoot, "deep", "b.bin"), It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        public async Task Run_Updates_Files_Increments_FilesUpdated_Creates_Parent_Dir()
        {
            ...
            copier.Verify(c => c.AtomicCopyAsync(J(fs, srcRoot, "x", "y", "z.txt"), J(fs, dstRoot, "x", "y", "z.txt"), It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        public async Task Run_Mixed_Buckets_All_Handled()
        {
            ...
            copier.Verify(c => c.AtomicCopyAsync(J(fs, srcRoot, "copy.txt"), J(fs, dstRoot, "copy.txt"), It.IsAny<CancellationToken>()), Times.Once);
            copier.Verify(c => c.AtomicCopyAsync(J(fs, srcRoot, "upd.txt"),  J(fs, dstRoot, "upd.txt"),  It.IsAny<CancellationToken>()), Times.Once);
        }
    }
}
